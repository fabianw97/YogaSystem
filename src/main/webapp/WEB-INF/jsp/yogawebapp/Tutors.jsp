...
<meta name="description" content="Tutors page">
...
<body>
		...
		<div class="container">
			...
			<div id="navbar" class="navbar-collapse collapse">
				<ul class="nav navbar-nav">
					...
					<li class="active"><a
						href="${pageContext.request.contextPath}/tutors">Tutors</a></li>
	...
	<div class="container">
		...
		<div class="jumbotron">
			<h1>Tutors</h1>
			<p>Here is a list of all the currently registered class Tutors.
				You can view the classes they offer and where by clicking on a tutors name.</p>

			<c:forEach var="tutor" items="${tutors}">
				<form id="form-id" action="tutors" method="GET">
					<input type="Hidden" name="tutor_id" id="tutorIdInput"
						value="${tutor.id}">
				</form>
				<a href="#" onclick="$('#form-id').submit();">
					${tutor.person.forenames}${tutor.person.surname},
					${tutor.person.email}</a>
			</c:forEach>
			<c:if test="${not empty tutorClasses}">
				<c:forEach var="c" items="${tutorClasses}">
					<table class="table table-bordered">
						...
						<tbody>
							<tr>
								<td>${c._class.class_type.className}</td>
								<td><strong>${c._class.location.name}</strong>,
									${c._class.location.address},
									${c._class.location.postcode}</td>
							<tr>
						</tbody>
					</table>
				</c:forEach>
			</c:if>
			<c:if test="${empty tutorClasses}">
				<p> This tutor doesn't have any classes available at the moment, check back later!</p>
			</c:if>
		</div>
